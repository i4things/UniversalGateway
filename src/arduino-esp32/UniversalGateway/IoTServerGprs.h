...
class IoTServerGprs
{
  public :
      ...
      DATA_SENT_CALLBACK(data_sent_callback_)
    {
      ...
      // force signal strength read
      GPRS_DO = true;

      GPRS_DNS_AT_INDEX = 14; // not used in SIM800L case

      GPRS_DATA_AT_INDEX = 17;
  ...
  private:
    ...
    inline bool GPRS_GET(char c[], char ret[], uint16_t & size)
    {
      ...
      {
        strcpy(ret, GPRS_SERVER);
        size = strlen(GPRS_SERVER);
      }
      else if (strcmp("SERVER_PORT", c) == 0)
      {
        itoa ((int)GPRS_SERVER_PORT, ret, 10);
        size = strlen(ret);
      }
      ...
      else if (strcmp("DATA", c) == 0)
      {
        ...
        //for SIM800L no need to add 0x0D at the end
#if defined (LOG64_ENABLED)
    ...
    inline bool GPRS_MOVE_TO_NEXT_COMMAND()
    {
      ...
      // we need to leave it going to first operation after signal strenghth to get signal strength
      else if ((GPRS_COMMAND_I == (GPRS_SIGNAL_INDEX + 1)) && (GPRS_QUEUE_BUF_SIZE == 0) && (GPRS_BUF_IN_NETWORK_SIZE == 0))
      {
        // clean soft reset count
        GPRS_SOFT_RESET_COUNT = 0;
      ...
      for (;;)
      {
        if (GPRS_COMMAND_J == 0)
        {
          ...
          if (GPRS_COMMAND[GPRS_COMMAND_I][0][0] != NULL)
          {
            ...
            if ((GPRS_COMMAND_I == GPRS_RECEIVE_AT_INDEX) || (GPRS_COMMAND_I == GPRS_DATA_AT_INDEX))
            {
              for (int i = 0; i < fi_s; i++)
              {
                LOG64_SET(String((uint8_t)fi[i], HEX));
                if (i > 8)
                {
                  LOG64_SET(F("..."));
                  break;
        ...
        case 1 : // set disconnect
          {
            ...
            {
              // fist start we want to keep it in HIGH - to not porform swith-of and on cycle
#if defined (LOG64_ENABLED)
              ...
              for ( uint16_t i  = 0; i <= GPRS_RECV_DATA_SIZE;)
              {
                ...
                else if (end_index == i)
                {
                  ...
                  else
                  {
                    ...
                    {
                      RECEIVED_CALLBACK(tmp_buf, 0, size);
                    }
                  }
  ...
  private:
    ...
    char GPRS_SERVER[254];
    uint32_t GPRS_SERVER_PORT;
    ...
    uint16_t GPRS_DATA_AT_INDEX;
    const char * GPRS_DATA_AT;
    uint16_t GPRS_DATA_AT_SIZE;
