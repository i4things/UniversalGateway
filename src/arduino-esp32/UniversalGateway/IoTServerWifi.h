...
#define IoTServerWifi_MAX_PACKET_WAIT 1000
#define IoTServerWifi_WAIT_TIMEOUT  1000
#define IoTServerWifi_DEFAULT_TIMEOUT 1000
#define IoTServerWifi_EXTENDED_TIMEOUT 30000
#define IoTServerWifi_CONNECT_TIMEOUT_DEFAULT  60000
#define IoTServerWifi_BUF_MAX_SIZE 1024
#define IoTServerWifi_MAX_MESSAGE_LEN 51

class IoTServerWifi
{
  public :
    IoTServerWifi(const char * ssid_,
                  const char * pass_,
                  const char * server_,
                  uint16_t server_port_,
                  void (* received_callback_)(uint8_t buf_[], uint8_t start_index_, uint8_t size_)  = NULL,
                  void (* server_time_callback_)(uint32_t time_)  = NULL,
                  void (* data_sent_callback_)(uint16_t size_) = NULL) :
      server_port(server_port_),
      received_callback(received_callback_),
      server_time_callback(server_time_callback_),
      data_sent_callback(data_sent_callback_)
    {
      strcpy(ssid, ssid_);
      strcpy(pass, pass_);
      strcpy(server, server_);
    }
    ...
    void init()
    {
      ...
      work_timeout = IoTServerWifi_DEFAULT_TIMEOUT;
    ...
    // set server and port ( in case after start needs to be changed)
    void set_server_port(const char * server_, uint16_t server_port_)
    {
      server_port = server_port_;
      strcpy(server, server_);
      flush();
    ...
    {
      if ((queue_buf_size + ((uint16_t)size)) > IoTServerWifi_BUF_MAX_SIZE)
      {
#if defined (LOG64_ENABLED)
  ...
  private:
    ...
    void decode()
    {
      for (; client.available() > 0;)
      {
        ...
        {
          if (decode_buf[0] > IoTServerWifi_MAX_MESSAGE_LEN)
          {
            //max length check
#if defined (LOG64_ENABLED)
        ...
        if (decode_buf_size == (decode_buf[0] + 1))
        {
          ...
          if  (decode_buf_size == 5)
          {
            ...
            LOG64_NEW_LINE;
#endif
            if (server_time_callback != NULL)
            {
              server_time_callback(server_time);
            }
          }
          else
          {
            ...
            {
              received_callback(decode_buf, 1, decode_buf_size - 1);
            }
          }
  ...
  public:
    ...
    void work()
    {
      ...
      if (response_wait)
      {
        if (((uint32_t)(((uint32_t)millis()) - response_wait_execute)) >= IoTServerWifi_WAIT_TIMEOUT)
        {
          work_last_execute = millis();
#if defined (LOG64_ENABLED)
      ...
      if ( WiFi.status() != WL_CONNECTED)
      {
        if (((uint32_t)(((uint32_t)millis()) - connect_last_execute)) >= connect_timeout)
        {
          ...
          connect_last_execute = millis();
          connect_timeout = IoTServerWifi_CONNECT_TIMEOUT_DEFAULT;
#if defined (LOG64_ENABLED)
        ...
        if ( WiFi.status() == WL_CONNECTED)
        {
          ...
          if (!client.connected())
          {
            ...
            if (!client.connect(server, server_port))
            {
              ...
              work_timeout = IoTServerWifi_EXTENDED_TIMEOUT;
          ...
          if (queue_buf_size_in_network > 0)
          {
            ...
            if ((queue_buf_size_in_network + queue_buf_size) <= IoTServerWifi_BUF_MAX_SIZE)
            {
          ...
          if (queue_buf_size > 0)
          {
            ...
            // for ESP32 skip and try multiple retry manually
            {
              ...
              for (uint32_t write_start = (uint32_t)millis();;)
              {
                ...
                else if (((uint32_t) (((uint32_t)millis()) -  write_start)) > IoTServerWifi_MAX_PACKET_WAIT )
                {
                  break;
                }
              ...
              else
              {
                ...
                {
                  data_sent_callback(queue_buf_size);
                }
              }
  ...
  private:
    ...
    uint8_t queue_buf[IoTServerWifi_BUF_MAX_SIZE];
    uint16_t queue_buf_size;

    uint8_t queue_buf_in_network[IoTServerWifi_BUF_MAX_SIZE];
    uint16_t queue_buf_size_in_network;

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //DECODE RECEIVE
    uint8_t decode_buf[IoTServerWifi_BUF_MAX_SIZE];
    ...
    void (* received_callback)(uint8_t buf_[], uint8_t start_index_, uint8_t size_);
    void (* server_time_callback)(uint32_t time_);
    void (* data_sent_callback)(uint16_t size_);
};
