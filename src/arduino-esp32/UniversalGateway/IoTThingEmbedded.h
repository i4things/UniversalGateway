...
//class IoTThingEmbedded
//{
//  public:
//    // constructor
//   IoTThingEmbedded(uint64_t id_,
//                   uint8_t key_[16],
//                   uint64_t gateway_id_,
//                   void (* received_)(uint8_t buf[], uint8_t size, int16_t rssi) // called when th ode wats to send data to the server ( need to be enveloped by gateway),  rssi = 0db/100%
//                   ) :
//
//    // call before using
//    void init();
//
//    // call from inside receive_callback_ - after schedule the data to be send to server 
//    void data_sent_successfully();
//
...
#if defined(IoTThing_MESSAGE_TIMEOUT)
#undef IoTThing_MESSAGE_TIMEOUT
#endif
...
class IoTThingEmbedded 
{
  public :
    ...
    IoTThingEmbedded(uint64_t id_,
                    uint8_t key_[16],
                     uint64_t gateway_id_,
                    void (* received_)(uint8_t buf[], uint8_t size, int16_t rssi) // called when the node wants to send data to the server ( need to be enveloped by gateway),  rssi = 0db/100%
                   ) :
      id(id_),
  ...
  public :
    ...
    void work()
    {
      ...
      if (q != NULL)
      {
        received(buf, buf_size, 0); // rssi - 100%
        total++;
        q = NULL;
        if ((++seq) > 126)
        {
  ...
  private :
    ...
    void (* received)(uint8_t buf_[], uint8_t size_, int16_t rssi_);
